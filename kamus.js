...
// Tulis ke file JSON dengan retry otomatis kalau kena konflik sha (409 "does not match").
// Dipakai khusus untuk index/<letter>.json karena file itu sering ditulis ulang
// beberapa fungsi dalam waktu berdekatan (tambah/ubah/hapus/pulihkan kata), jadi
// sha yang diambil sebelumnya gampang basi.
// mutateFn menerima data lama (idxData, bisa null kalau file belum ada) dan
// mengembalikan data baru untuk disimpan. Kalau mutateFn mengembalikan null/undefined,
// dianggap "tidak perlu simpan" (skip, konsisten dengan pengecekan `if (idxData)` lama).
async function putFileJsonWithRetry(path, mutateFn, message, maxRetries = 3) {
  let lastErr;
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    ...
    try {
      ...
      lastErr = e;
      if (!/does not match/i.test(e.message)) throw e; // bukan konflik sha — lempar langsung
      // konflik sha: ulangi loop, ambil sha terbaru lalu coba lagi
    }
  }
  throw lastErr;
}
...
async function addWord(entry) {
  ...
  // update index/<letter>.json — dengan retry kalau sha basi
...
async function editWord(letter, slug, updates) {
  ...
  // sinkronkan meaning di index kalau berubah — dengan retry kalau sha basi
  await putFileJsonWithRetry(
    `index/${letter}.json`,
    (idxData) => {
      if (!idxData) return null;
      return idxData.map(item =>
...
async function restoreWordVersion(letter, slug, commitSha) {
  ...
  // sinkronkan index/<letter>.json (tambahkan lagi kalau sebelumnya terhapus, atau update
  // meaning-nya) — dengan retry kalau sha basi
  await putFileJsonWithRetry(
    `index/${letter}.json`,
    (idxData) => {
      let idx = idxData || [];
