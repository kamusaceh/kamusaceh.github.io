...
async function fetchStats() {
  const res = await fetch(rawUrl(`stats/global.json`));
  if (!res.ok) return { wordCount: 0, editCount: 0 };
  return await res.json();
}
...
/* ---------- 8d. STATISTIK GLOBAL --- */
async function bumpStats(deltaWord, deltaEdit) {
  try {
    const { sha, data } = await getFileJson(`stats/global.json`);
    const stats = data || { wordCount: 0, editCount: 0 };
    stats.wordCount = Math.max(0, (stats.wordCount || 0) + deltaWord);
    stats.editCount = (stats.editCount || 0) + deltaEdit;
    await putFileJson(`stats/global.json`, stats, "Update statistik", sha);
  } catch (e) {
    console.warn("Gagal update statistik:", e.message);
  }
}

/* ---------- 9. RENDER --- */
function renderEntri(container, data, slug, letter) {
  const posHtml = data.pos ? `<span class="pos">${escapeHtml(data.pos)}</span>` : "";
  const contohHtml = (data.examples && data.examples.length)
    ? data.examples.map(ex => `<div class="contoh"><p class="aceh">${escapeHtml(ex.aceh)}</p><p class="indo">${escapeHtml(ex.indo)}</p></div>`).join("")
    : "<p class=\"meta\">Belum ada contoh kalimat.</p>";
  const rujukanHtml = (data.rujukan && data.rujukan.length)
    ? data.rujukan.map(r => `<a href="/entri/${slugify(r)}" class="rujukan-chip" data-slug="${slugify(r)}" data-letter="${firstLetter(r)}">${escapeHtml(r)}</a>`).join("")
    : "";
  const sumberHtml = data.sumber ? `<p class="meta">Sumber: ${escapeHtml(data.sumber)}</p>` : "";
  container.innerHTML = `<div class="entri"><h1>${escapeHtml(data.word)}</h1>${posHtml}<p class="meaning">${escapeHtml(data.meaning)}</p>${contohHtml}${rujukanHtml}${sumberHtml}</div>`;
}
