...
function findExistingBySlug(slug){
  return currentIndex.find(e => e.slug === slug);
}

// Peringatan real-time saat mengetik kata baru (bukan saat sedang edit kata yang sudah ada).
function checkDuplicateWord(){
  const box = document.getElementById('dupWarning');
  if(!formState || formState._slug){ box.style.display = 'none'; return; } // sedang edit kata lama, bukan tambah baru
  const slug = slugify(formState.word || '');
  if(!slug){ box.style.display = 'none'; return; }
  const existing = findExistingBySlug(slug);
  if(existing){
    box.className = 'status err';
    box.style.display = 'block';
    box.innerHTML = `Kata "${escapeHtml(existing.word)}" sudah ada di kamus. <a href="#" onclick="openWord('${existing.slug}'); return false;">Buka & edit kata ini</a> daripada menambah duplikat.`;
  }else{
...
async function connect(){
  ...
  try{
    ...
    document.getElementById('connStatusText').textContent = `${c.owner}/${c.repo}`;
    document.getElementById('actionCards').style.display = 'flex';
  }catch(e){
...
function toggleAdvancedConfig(){
  ...
  document.getElementById('advToggleBtn').textContent = shown ? 'Pengaturan lanjutan ▾' : 'Pengaturan lanjutan ▴';
}

// ===================== MODE KERJA (Tambah / Cari&Edit / Riwayat) =====================
...
function setMode(mode){
  ...
  document.getElementById('actionBtn_' + mode).classList.add('active');

  const editSearchCard = document.getElementById('editSearchCard');
  const formCard = document.getElementById('formCard');
  const editEmptyHint = document.getElementById('editEmptyHint');
  const historyArea = document.getElementById('historyArea');

  editSearchCard.style.display = 'none';
  formCard.style.display = 'none';
  editEmptyHint.style.display = 'none';
  ...
  if(mode === 'add'){
    ...
    editSearchCard.style.display = 'block';
    document.getElementById('editSearch').value = '';
    document.getElementById('searchSuggestions').innerHTML = '';
    if(formState && formState._slug) formCard.style.display = 'block';
    else editEmptyHint.style.display = 'block';
...
function renderSearchSuggestions(query){
  ...
  const results = currentIndex
    .filter(e => e.word.toLowerCase().includes(q))
    .sort((a, b) => a.word.localeCompare(b.word))
    .slice(0, 8);
  if(results.length === 0){ box.innerHTML = '<div class="empty">Tidak ada kata yang cocok.</div>'; return; }
  box.innerHTML = results.map(e => `
    <div class="suggestion-item" onclick="selectSearchResult('${e.slug}')">
...
function renderInfoPanel(){
  ...
  document.getElementById('infoTotal').textContent = `${currentIndex.length}`;
}

async function loadLastUpdateInfo(){
  const c = cfg();
  const el = document.getElementById('infoLastUpdate');
  el.textContent = 'Memuat...';
  try{
    const commits = await ghListCommits(c, `${c.dataPrefix}/index.json`);
    if(commits.length && commits[0].commit.author){
      el.textContent = new Date(commits[0].commit.author.date).toLocaleString('id-ID');
    }else{
      el.textContent = '–';
    }
  }catch(e){
...
// Catatan: daftar kata statis (#wordList) diganti dengan pencarian bertingkat
// (renderSearchSuggestions) di mode "Cari & Edit" — lihat bagian MODE KERJA di atas.
...
async function saveWord(){
  ...
  const slug = isNew ? slugify(formState.word) : formState._slug;

  if(isNew){
    const dup = findExistingBySlug(slug);
    if(dup){
      showStatus('formStatus', `Kata "${dup.word}" sudah ada di kamus. Buka kata itu untuk mengeditnya, jangan menambah duplikat.`, 'err');
      return;
    }
  }
  ...
  try{
    ...
    renderInfoPanel();
    document.getElementById('infoLastUpdate').textContent = new Date().toLocaleString('id-ID');
